...
class Person{
	...
	public:
		Person(string n = " ", int a = 0, char g = 'M', int y = 1, int m = 1, int d = 1):date(y, m, d)//对象成员的初始化放在初始化列表后,要注意对象成员中所需的参数要与初始化列表中的一致
		{
			name = n;
			age = a;
			gender = g;
...
class Student:public Person
{
	int grade,score;
	public:
		Student(string n = " ", int a = 0, char g = 'M', int y = 1, int m = 1, int d = 1, int grade = 1, int s = 0):Person(n, a, g, y, m, d)
		{
		    this->grade = grade;
		    score = s;
		    cout<<"Student类的构造"<<endl;
		}
		~Student()
		{
